...
```mermaid
flowchart TD
    classDef client fill:#cff,stroke:#333;
    classDef control fill:#fcf,stroke:#333;
    classDef proxy fill:#9f9,stroke:#333;
    classDef backend fill:#f99,stroke:#333;

    subgraph Node
        Client[Client]:::client --> ServiceIP["Service IP (IPTables)"] 
        ServiceIP --> KubeProxy[kube-proxy]:::proxy
    end
    APIServer[APIServer]:::control --> KubeProxy
    KubeProxy --> Backend1["backend1(labels: app=myapp)"]:::backend
    KubeProxy --> Backend2["backend2(labels: app=myapp)"]:::backend
    KubeProxy --> Backend3["backend3(labels: app=myapp)"]:::backend

    linkStyle 0,1 stroke:#09f,stroke-width:2px
    linkStyle 2 stroke:#f90,stroke-width:2px
    linkStyle 3,4,5 stroke:#f09,stroke-width:1.5px
```
...
flowchart TD
    ...
    subgraph Node
        Client[Client]:::client --> ServiceIP["Service IP (IPTables)"] 
        KubeProxy[kube-proxy]:::proxy --> ServiceIP
    end
    APIServer[APIServer]:::control --> KubeProxy
    ServiceIP --> Backend1["backend1(labels: app=myapp)"]:::backend
    ServiceIP --> Backend2["backend2(labels: app=myapp)"]:::backend
    ServiceIP --> Backend3["backend3(labels: app=myapp)"]:::backend
